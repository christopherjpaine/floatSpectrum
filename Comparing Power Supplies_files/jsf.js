...
var isIECache;
var getIEVersion=function getIEVersion(){if(typeof IEVersionCache!=="undefined"){return IEVersionCache
}if(/MSIE ([0-9]+)/.test(navigator.userAgent)){IEVersionCache=parseInt(RegExp.$1)
}else{IEVersionCache=-1
...
var getFormForId=function getFormForId(id){if(id){var node=document.getElementById(id);
while(node){if(node.nodeName&&(node.nodeName.toLowerCase()=="form")){return node
...
if(!!type&&type[1]){if(type[1]!=="text/javascript"){continue
}}var src=[];
...
var isIE9Plus=function isIE9Plus(){var iev=getIEVersion();
if(iev>=9){return true
}else{return false
}};
var deleteNode=function deleteNode(node){if(!node){return
}if(!node.parentNode){return
}if(!isIE()||(isIE()&&isIE9Plus())){node.parentNode.removeChild(node);
return
...
var propertyToAttribute=function propertyToAttribute(name){if(name==="className"){return"class"
...
var detectAttributes=function detectAttributes(element){if(element.hasAttribute&&isFunctionNative(element.hasAttribute)){return function(name){return element.hasAttribute(name)
}
}else{try{element.getAttribute;
...
var cloneAttributes=function cloneAttributes(target,source){var coreElementProperties=["className","title","lang","xmllang"];
...
iIndex<iLength;
iIndex++){var propertyName=propertyNames[iIndex];
var attributeName=propertyToAttribute(propertyName);
if(sourceAttributeDetector(attributeName)){if(attributeName=="class"){if(isIE()&&(source.getAttribute(propertyName)===source[propertyName])){attributeName=propertyName
...
}}else{if(attributeName=="value"&&(target.type!="checkbox"&&target.type!="radio")){target[propertyName]=""
...
jIndex<jLength;
jIndex++){var booleanPropertyName=booleanPropertyNames[jIndex];
...
}}if(sourceAttributeDetector("style")){var newStyle;
var oldStyle;
if(isIE()){newStyle=source.style.cssText;
oldStyle=target.style.cssText;
if(newStyle!=oldStyle){target.style.cssText=newStyle
}}else{newStyle=source.getAttribute("style");
oldStyle=target.getAttribute("style");
if(newStyle!=oldStyle){target.setAttribute("style",newStyle)
}}}else{if(targetAttributeDetector("style")){target.removeAttribute("style")
}}if(!isIE()&&source.dir!=target.dir){if(sourceAttributeDetector("dir")){target.dir=source.dir
}else{if(targetAttributeDetector("dir")){target.dir=""
}}}for(var lIndex=0,lLength=listenerNames.length;
lIndex<lLength;
lIndex++){var name=listenerNames[lIndex];
target[name]=source[name]?source[name]:null;
if(source[name]){source[name]=null
}}try{var targetDataset=target.dataset;
var sourceDataset=source.dataset;
if(targetDataset||sourceDataset){for(var tp in targetDataset){delete targetDataset[tp]
}for(var sp in sourceDataset){targetDataset[sp]=sourceDataset[sp]
}}}catch(ex){}};
...
var getEncodedUrlElement=function getEncodedUrlElement(form){var encodedUrlElement=form["javax.faces.encodedURL"];
if(encodedUrlElement){return encodedUrlElement
}else{var formElements=form.elements;
for(var i=0,length=formElements.length;
i<length;
i++){var formElement=formElements[i];
if(formElement.name&&(formElement.name.indexOf("javax.faces.encodedURL")>=0)){return formElement
}}}return undefined
};
var getViewStateElement=function getViewStateElement(form){var viewStateElement=form["javax.faces.ViewState"];
if(viewStateElement){return viewStateElement
}else{var formElements=form.elements;
for(var i=0,length=formElements.length;
i<length;
i++){var formElement=formElements[i];
...
if(id==="javax.faces.ViewState"){state=jQuery(element).text();
if(typeof context.formid!=="undefined"&&context.formid!==null){stateForm=getFormForId(context.formid)
}else{stateForm=getFormForId(context.element.id)
...
if(typeof f!=="undefined"&&f!==null&&f.id!==context.formid){field=getViewStateElement(f);
if(typeof field==="undefined"){field=document.createElement("input");
field.type="hidden";
field.name="javax.faces.ViewState";
f.appendChild(field)
...
if(name==="disabled"){target.disabled=value==="disabled"||value==="true";
return
}else{if(name==="checked"){target.checked=value==="checked"||value==="on"||value==="true";
return
}else{if(name=="readonly"){target.readOnly=value==="readonly"||value==="true";
return
}}}if(!isIE()){if(name==="value"){target.value=value
}else{target.setAttribute(name,value)
}}else{if(name==="class"){target.className=value
...
}else{if(name.substring(0,2)==="on"){var c=document.body.appendChild(document.createElement("span"));
try{c.innerHTML="<span "+name+'="'+value+'"/>';
...
var AjaxEngine=function AjaxEngine(context){var req={};
req.url=null;
req.context=context;
...
if(typeof(namingContainerId)==="undefined"||options===null){namingContainerId=""
}args[namingContainerId+"javax.faces.source"]=element.id;
if(event&&!!event.type){args[namingContainerId+"javax.faces.partial.event"]=event.type
...
for(var property in options){if(options.hasOwnProperty(property)){if(property!="com.sun.faces.namingContainerId"){args[namingContainerId+property]=options[property]
...
if(responseType.nodeName==="error"){var errorName="";
var errorMessage="";
var element=responseType.firstChild;
if(element.nodeName==="error-name"){if(null!=element.firstChild){errorName=element.firstChild.nodeValue
}}element=responseType.firstChild.nextSibling;
if(element.nodeName==="error-message"){if(null!=element.firstChild){errorMessage=element.firstChild.nodeValue
...
var i;
var j;
while(h<g.length){if(typeof g[h].src==="string"&&g[h].src.match("/javax.faces.resource/jsf.js?.*ln=javax.faces")){f=g[h].src;
break
}h++
}if(typeof f=="string"){j=f.match("stage=(.*)");
if(j){i=j[1]
}}if(typeof i==="undefined"||!i){i="Production"
}mojarra=mojarra||{};
mojarra.projectStageCache=i;
return mojarra.projectStageCache
};
jsf.getViewState=function(q){if(!q){throw new Error("jsf.getViewState:  form must be set")
}var o=q.elements;
var l=o.length;
var i=[];
var r=function(b,a){var c="";
if(i.length>0){c="&"
...
m++){var p=o[m];
if(p.name===""){continue
}if(!p.disabled){switch(p.type){case"submit":break;
case"image":break;
case"select-one":if(p.selectedIndex>=0){r(p.name,p.options[p.selectedIndex].value)
}break;
case"select-multiple":for(var n=0;
n<p.options.length;
n++){if(p.options[n].selected){r(p.name,p.options[n].value)
}}break;
case"checkbox":case"radio":if(p.checked){r(p.name,p.value||"on")
}break;
default:var j=p.nodeName.toLowerCase();
if(j==="input"||j==="select"||j==="button"||j==="object"||j==="textarea"){r(p.name,p.value)
}break
}}}return i.join("")
};
jsf.util={};
jsf.util.chain=function(i,k){if(arguments.length<3){return true
}var h=(typeof i==="object")?i:null;
for(var f=2;
f<arguments.length;
f++){var j=new Function("event",arguments[f]);
var l=j.call(h,k);
...
mojarra.dpf=function dpf(f){var d=f.adp;
if(d!==null){for(var e=0;
e<d.length;
e++){f.removeChild(d[e])
}}};
mojarra.apf=function apf(j,l){var k=new Array();
j.adp=k;
var f=0;
for(var h in l){if(l.hasOwnProperty(h)){var i=document.createElement("input");
i.type="hidden";
i.name=h;
i.value=l[h];
j.appendChild(i);
...
mojarra.jsfcljs=function jsfcljs(i,j,f){mojarra.apf(i,j);
var h=i.target;
if(f){i.target=f
}if(i.onsubmit){var g=i.onsubmit();
if((typeof g=="undefined")||g){i.submit()
}}else{i.submit()
}i.target=h;
mojarra.dpf(i)
};
mojarra.jsfcbk=function jsfcbk(d,e,f){return d.call(e,f)
};
mojarra.ab=function ab(l,k,i,h,e,j){if(!j){j={}
}if(i){j["javax.faces.behavior.event"]=i
}if(h){j.execute=h
}if(e){j.render=e
}jsf.ajax.request(l,k,j)
};